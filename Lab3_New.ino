...
/* 
The running average filter helps to smooth out readings, like sensor data, by averaging a set number 
of past values. This makes the output less sensitive to sudden changes and helps to give a clearer 
picture over time.
*/
...
void setup() {
  ...
  clientTCP.setCACert(TELEGRAM_CERTIFICATE_ROOT); // Add root certificate for Telegram
  ...
  if (WiFi.status() == WL_CONNECTED) {
    WiFi.softAP((WiFi.localIP().toString() + "_" + apssid).c_str(), appassword);
    Serial.println("Connected to Wi-Fi");
    Serial.print("ESP32-CAM IP Address: ");
    Serial.println(WiFi.localIP());
  } else {
    WiFi.softAP((WiFi.softAPIP().toString() + "_" + apssid).c_str(), appassword);
...
/* Xử lý các requests HTTP cho quá trình chụp ảnh từ camera */
...
static esp_err_t stream_handler(httpd_req_t *req){
    ...
    while(true){
        ...
        Serial.printf("MJPG: %uB %ums (%.1ffps)t, AVG: %ums (%.1ffps), %u+%u+%u+%u=%u %s%d\n",
...
<html>
    <head>
        ...
        <title>Teachable Machine-Nhóm 3</title>
        <style>
    ...
    <body>
        <section class="main">
            ...
            <h3>Lab 3 - Nhóm 9: Hồ Văn Phú & Đỗ Nguyên Phương</h3>        
            <section id="buttons">
                <table>
